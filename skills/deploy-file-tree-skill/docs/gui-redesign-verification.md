...
前端 112 项测试、查看器 123 项测试、部署器 32 项测试，以及 typecheck、build 和文件树严格检查通过。既有虚拟列表测试会输出 React `flushSync` 警告；基线已存在，测试仍通过。

审查后的回归补充：分隔条仅处理无修饰键的左右调宽，避免截获 Alt 历史导航；刷新换代后重查最新已提交搜索，并对晚回的旧代查询作一次有界重试，防止覆盖后续查询。新增三项契约测试覆盖这些路径；本段记录自动化测试结果，真实浏览器复验另行补充。
