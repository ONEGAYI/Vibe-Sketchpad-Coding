...
- 跨环境路径（G20–G23）：旧机 SCP/SFTP 复制 JSON（无需 Python）→ 现代机 viewer.py 浏览；旧机运行核心工具需兼容 Python（CentOS 7 系统 3.6.8 实测拒绝运行，独立解释器候选与证据见 [跨环境验证记录](references/compatibility.md)）
