...
dist/ 是 file-tree 技能的发行版快照——公用核心六件套
（SKILL.md / references/views.md / references/viewer.md / agents/openai.yaml /
...
def update_dist(
    ...
    """从源仓库提取固定清单（十一件）刷新 dist 快照（不动 dist 里其他内容）。"""
...
def main(argv=None) -> int:
    ...
    p = sub.add_parser("update-dist", help="从源仓库提取固定清单（十一件）刷新 dist 快照")
