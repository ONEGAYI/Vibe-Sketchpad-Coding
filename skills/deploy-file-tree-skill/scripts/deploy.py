...
（.history.json）永不动。tree.json 的升级判定与 check 同源（is_canonical_text）：
新旧两种规范排版（紧凑 / 两空格缩进）均为有效数据，部署不重写任何字节，
留待下次正常写入时自动转换为新紧凑格式；仅当结构不规范（如缺派生 kind
字段）才做规范化结构迁移（数据语义不变），且迁移经统一写入口直接输出
新紧凑规范，保证数据无损且废弃文件升级到位。
...
def deploy(
    ...
    try:
        ...
        if fresh:
            ...
            # 升级判定与 check 同源（is_canonical_text）：新旧两种规范排版均为
            # 有效数据，不重写任何字节，留待下次正常写入时自动转换为新紧凑
            # 格式；仅当结构不规范（缺派生 kind 等）才做规范化结构迁移，
            # 且经统一写入口直接输出新紧凑规范
            raw = tool.tree_json.read_text(encoding="utf-8")
            if ft.is_canonical_text(raw):
                if raw.replace("\r\n", "\n") != ft.dumps_canonical(
                    ft.normalize_data(tool.load())
                ):
