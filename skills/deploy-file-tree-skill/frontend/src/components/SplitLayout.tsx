...
export function SplitLayout({ children }: { children: ReactNode }) {
  ...
  useLayoutEffect(() => {
    if (geometry.stacked) finishDrag();
  }, [geometry.stacked]);

  return (
    <main ref={root} className={`columns${dragging ? " resizing" : ""}`} data-stacked={geometry.stacked}
      style={{ "--sidebar-width": `${geometry.width}px` } as React.CSSProperties}>
      {panels[0]}
      <div ref={divider} className="sidebar-divider" role="separator" aria-label="调整目录栏宽度"
        aria-orientation="vertical" aria-valuemin={geometry.min} aria-valuemax={geometry.max}
        aria-valuenow={geometry.width} aria-disabled={geometry.stacked} tabIndex={geometry.stacked ? -1 : 0}
        onPointerDown={(event) => {
          if (geometry.stacked || event.button !== 0) return;
        ...
        onPointerMove={(event) => {
          const active = drag.current;
          if (!active || active.pointer !== event.pointerId || geometry.stacked) return;
        ...
        onKeyDown={(event) => {
          if (geometry.stacked || !["ArrowLeft", "ArrowRight"].includes(event.key)) return;
          event.preventDefault();
