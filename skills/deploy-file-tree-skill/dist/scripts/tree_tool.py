...
class TreeTool:
    ...
    def _view_renderable(self, view_id: str, spec: dict, data: dict, quiet: bool = False) -> tuple[str, list[str]] | None:
        """返回 (剪影内容, 锚点段)；不可渲染（锚点悬空/过滤器超范围/锚点非目录）时
        打印告警并返回 None——渲染管线跳过该视图而非整体失败，坏配置留给
        view-list 呈现与后续 check 诊断。quiet=True 供 check 复用求值而不打
        渲染期告警（check 以自己的两级消息呈现）。
        """
    ...
    def check(self, strict: bool = False) -> tuple[list[str], list[str]]:
        ...
        for view_id, spec in sorted(views.items(), key=lambda kv: sort_key(kv[0])):
            ...
            if made is None:
                warnings.append(
                    f"W: 视图 {view_id} 当前不可渲染（过滤器非单 under 锚点或锚点不在树中），跳过其绑定文档的块内容比对"
                )
            for doc_rel in spec.get("docs", []):
                try:
                ...
                if n_end == 0:
                    ...
                    continue
                if made is not None and block_content(disk_doc, begin, end) != made[0]:
                    errors.append(
