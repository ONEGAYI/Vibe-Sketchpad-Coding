...
view-add <id> --under <目录> [--doc <路径> [--line N]]
                                  # 登记视图（id：[a-z0-9][a-z0-9_-]{0,63}，禁保留字 default）：把目录锚点
                                  # 子树的剪影渲染到绑定文档的带 id 标记块；--line 为围栏首行承诺落点
                                  # （1-based，插在当前内容前，越界报错，省略追加尾部）；同 id 重复执行
                                  # = upsert（改锚点/换文档/--line 重定位）；单步撤销历史
...
**子树视图（多文档镜像）**：`view-add` 登记的视图把目录锚点子树的剪影（根到锚点的目录链只作容器不显示简介，首行为全局 root 名，hidden 条目不出现）渲染到各绑定文档的带 id 标记块（`<!-- file-tree:tree^id=<id>:begin ... -->`，代码围栏包裹，仅树块无 tags 块）。视图配置存于 tree.json 顶层 `views` 键（id + 过滤器 + 绑定文档清单），随数据变更自动刷新；数据写操作与 undo/redo 同样联动重渲染全部视图。一个视图 id 可绑定**一个或多个文档**（`view-doc --add` 增量扩充）：全部绑定文档中的块内容完全相同（同一渲染产物镜像），任何数据变更后全部镜像同步刷新；`view-doc --rm` 解绑但保留文档中的块（孤儿状态不再刷新，重复绑定时报错拒绝）。同一文档内同 id 出现多于一个块属病态：view-doc、view-add 与触发渲染的数据命令都会报错并指明文档路径与块数，不做猜测性修复（手改删除多余块后重跑 `render` 恢复）。无 `views` 的仓库行为与历史版本完全一致。
