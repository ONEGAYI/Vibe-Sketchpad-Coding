...
**子树视图（多文档）**：`view-add` 登记的视图把过滤器选中集的剪影渲染到各绑定文档的带 id 标记块（`<!-- file-tree:tree^id=<id>:begin ... -->`，代码围栏包裹，仅树块无 tags 块）。过滤器是 `and`/`or`/`not`/`under`/`tag` 五种节点的表达式树，在 tree.json 全量条目上求值（`not` 为全量补集语义，`under` 锚点自身含入）；剪影把选中集投回全树结构——选中条目正常渲染（简介照常、collapsed 生效），仅含选中后代的未选中祖先目录作路径骨架（只作容器不显示简介、不折叠），不连通选中集（散文件与多目录混合）按同一投影规则收敛为一棵树，首行为全局 root 名，hidden 条目在任何视图不出现，空选中集渲染仅剩根名行。视图配置存于 tree.json 顶层 `views` 键（id + 过滤器 + 绑定文档清单），随数据变更自动刷新；数据写操作与 undo/redo 同样联动重渲染全部视图。无 `views` 的仓库行为与历史版本完全一致。
