...
dist 只含**公用内容**（固定清单十一件 + `viewer/` 静态资源动态清单），不含任何仓库数据——`tree.json`（文件树数据）、`.history.json`（本机撤销历史）、AGENTS.md（目标仓库的规则文档）都属于各仓库私有。
...
- **首次部署**：复制固定清单十一件（主入口、两份按需参考、元数据、核心脚本与查看器脚本）与 `dist/viewer/` 静态资源 → 初始化空 `tree.json`（新紧凑规范格式）→ 渲染 AGENTS.md 标记块（无文件则生成骨架）→ 自动 `check` 自检。
