...
description: 将 file-tree 技能部署或升级到指定仓库，或应急从部署实例回收发行快照。不用于日常维护 tree.json 条目或只读浏览快照。
---
...
├── references/
│   └── compatibility.md   # 旧环境解释器与浏览器验证记录
├── scripts/
...
**构建与运行的依赖是分开的**：构建前端需要现代构建机的 Node（npm install + vite build）；部署后的目标机器运行查看器需要 **Python 3.8+ 与现代浏览器**——发行页面资源随技能入库与部署，目标无需 npm install、无需 vite build、无需 Node 在 PATH。
...
核心工具需要兼容其语法的 Python；只读查看器需要 Python 3.8+ 与现代浏览器。旧环境解释器候选、已验证环境和待验证范围见 [跨环境与浏览器验证记录](references/compatibility.md)。
...
4. 同步本机使用副本：复制主仓库的母体文件（SKILL.md、references/、scripts/）与 dist 公用内容（含 `viewer/`）到 `~/.agents/skills/deploy-file-tree-skill/`，保持调用入口与主线一致。

> 旧主线（QuotaTray `.agents/skills/file-tree/`）自批量命令版本（PR #47 合并）起降级为普通部署点。update-dist 保留应急语义：从任意部署实例提取当前固定清单回收散落改动。
