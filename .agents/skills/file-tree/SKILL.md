...
**子树视图（多文档）**：`view-add` 登记的视图把目录锚点子树的剪影（根到锚点的目录链只作容器不显示简介，首行为全局 root 名，hidden 条目不出现）渲染到各绑定文档的带 id 标记块（`<!-- file-tree:tree^id=<id>:begin ... -->`，代码围栏包裹，仅树块无 tags 块）。视图配置存于 tree.json 顶层 `views` 键（id + 过滤器 + 绑定文档清单），随数据变更自动刷新；数据写操作与 undo/redo 同样联动重渲染全部视图。无 `views` 的仓库行为与历史版本完全一致。
